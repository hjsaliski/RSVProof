...
async function sendReminder(attendee, event) {
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 420px; margin: 0 auto;">
          <p style="text-transform: uppercase; letter-spacing: 0.1em; font-size: 12px; color: #a9740f;">Reminder</p>
          <h1 style="font-size: 22px; margin: 0 0 4px;">${event.name}</h1>
          <p style="color: #5b574c; margin: 0 0 16px;">${eventDateDisplay} &middot; ${event.location}</p>
          <div style="background: #fbeecb; border-radius: 10px; padding: 14px; font-size: 14px; margin-bottom: 16px;">
            Your event is coming up. Check in when you arrive using the QR
            code from your confirmation email or screenshot, and your
            ${depositDisplay} hold is released. Miss check-in, and it's charged.
          </div>
        </div>
      `,
    });
...
async function processEvent(event) {
  ...
  for (const attendee of eligible || []) {
    results.push(await sendReminder(attendee, event));
  }
  return results;
}
